...
export class AdminDashboardComponent {
  sa_codes_api_data: string[] = [];
  coordinate_lookup: { [index: string]: { lat: number; long: number } } = {};
  ...
  wealth: number = 0;
  birthRates: number = 0;
  commercial_activity: number = 0;

  constructor(private utils: UtilsService, private http: HttpClient) {
    let csv_records: string[] = [];
    this.utils.readCSV().subscribe({
      next: (data) => {
        console.log('csv_data_records ===> \n', ...data);
        csv_records = data;
        this.prepareCoordinateLookup(csv_records);
      },
    });
    this.getAPIData();
  }
  ...
  getAPIData() {
    ...
    const url: string = `https://25uu0avefe.execute-api.ap-southeast-2.amazonaws.com/default?birth_weight=${birth_weight}&gum_weight=${gum_weight}&pop_weight=${pop_weight}&ses_weight=${ses_weight}`;
    this.http
      .post(url, null, { headers: headers, responseType: 'text' })
      .subscribe({
        next: (response) => {
          console.log('API Response ===>', response);
          apiData = response.split(',');
          console.log('API Response ===>', apiData);
          this.sa_codes_api_data = apiData;
  ...
  prepareCoordinateLookup(csv_records: string[]) {
    for (const record of csv_records) {
      const record_data = record.split(',');
      const lat: number = parseFloat(record_data[2]);
      const long: number = parseFloat(record_data[1]);
      this.coordinate_lookup[record_data[0] as string] = {
        lat: lat,
        long: long,
      };
      ...
      loader
        ...
        .then(({ Map }) => {
          ...
          heatmap = new google.maps.visualization.HeatmapLayer({
            data: getPoints(this.sa_codes_api_data, this.coordinate_lookup),
            map: map,
          });
        ...
        .catch((e) => {
          // do something
          console.error('error while loading the map =====> ', e);
        });
    } catch {
      console.log('some error occured');
    }
...
function getPoints(
  api_sa_codes: string[],
  coordinates_lookup: { [index: string]: { lat: number; long: number } }
) {
  let heatMapPoint: google.maps.LatLng[] = [];
  for (const sa_code of api_sa_codes) {
    if (coordinates_lookup[sa_code]) {
      const cords: { lat: number; long: number } = coordinates_lookup[sa_code];
      heatMapPoint.push(new google.maps.LatLng(cords?.lat, cords?.long));
    } else {
      console.log('SA code : ', sa_code, coordinates_lookup[sa_code]);
    }
  }
  console.log('Heat Map ===> ', heatMapPoint);
  return heatMapPoint;
  // return [
